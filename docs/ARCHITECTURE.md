...
Brand artwork is maintained as matched monochrome SVG geometry. The GitHub README uses `prefers-color-scheme` sources, while the DLL embeds clean transparent PNG rasterizations and selects the light or dark mark from SimHub's foreground/background luminance. This avoids bitmap edge halos without adding an SVG runtime dependency to the .NET Framework plugin.
